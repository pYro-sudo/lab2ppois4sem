...
public class Executor implements ExecutorActions{
    ...
    public List<Form> getFormsByMotherSalary(float minSalary, float maxSalary) {
        try{
            ...
            entityTransaction.begin();
            List<Form> formEntities = entityManager
                    .createQuery("select f from Form f where f.mother.salary >= :minSalary and f.mother.salary <= :maxSalary", Form.class)
    ...
    public List<Form> getFormsByFatherSalary(float minSalary, float maxSalary) {
        try{
            ...
            entityTransaction.begin();
            List<Form> formEntities = entityManager
                    .createQuery("select f from Form f where f.father.salary >= :minSalary and f.father.salary <= :maxSalary", Form.class)
                    .setParameter("minSalary", minSalary)
                    .setParameter("maxSalary", maxSalary)
                    .getResultList();
            entityTransaction.commit();
    ...
    @Override
    @Transactional
    public List<Form> deleteFormsByAmountOfSisters(int amount) {
        try{
            initEntityManagerFactory();
            initEntityManager();
            initEntityTransaction();
    ...
    @Override
    @Transactional
    public List<Form> deleteFormsByAmountOfBrothers(int amount) {
        try{
            initEntityManagerFactory();
            initEntityManager();
            initEntityTransaction();
